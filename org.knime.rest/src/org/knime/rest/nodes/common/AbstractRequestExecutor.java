...
public abstract class AbstractRequestExecutor<S extends RestSettings> extends AbstractCellFactory {
     ...
     */
    AbstractRequestExecutor(final MultiResponseHandler handler,final DataTableSpec spec, final DataColumnSpec[] colSpec,
        final S settings, final CooldownContext cooldown,
     ...
     */
    private static Throwable getRootIOCause(final Exception e) {
     ...
     * @return
     */
    private static String getReadableIOCauseMessage(final Throwable cause, final URI currentURI) {
     ...
     * <p>
     * Since we are using CXF currently for REST requests, the behavior of this request
     * is strongly dependent on the global configuration in the KNIMECXFBusFactory, and the
     * local configuration in {@link RestNodeModel#createClient()}.
     *
     * @param spec
     * @param settings
     * @param invocation
    ...
    private ResultPair performSingleRequest(final DataRow row)
        ...
        try {
            ...
            final var cause = getRootIOCause(e);
     ...
     * @throws InvalidSettingsException
     */
    public abstract Optional<InvocationTriple> createInvocationTriple(final DataRow row)
            throws InvalidSettingsException;
    ...
    /**
     * Returns the initial {@link DataTableSpec}. Essential for processing the responses.
     *
    ...
    public DataCell[] makeFirstCall(final DataRow row) throws InvalidSettingsException {
        ...
        try {
            ...
            if (response != null) { // NOSONAR it can be null
                try {
    ...
    private void sleepWithMonitor(final long duration, final LongPredicate condition)
            throws CanceledExecutionException {
        m_monitor.checkCanceled();
        for (var remaining = duration; remaining > 0; remaining -= CHECK_INTERVAL_MS) {
            try {
                Thread.sleep(Math.min(duration, CHECK_INTERVAL_MS));
                m_monitor.checkCanceled();
                if (condition.test(remaining)) {
                    return;
                }
    ...
    public interface MultiResponseHandler {
         ...
         * @return list of data cells, containing the parsed data
         */
        DataCell[] handleFirstResponse(final DataTableSpec spec, final Response response, final MissingCell missing);
         ...
         *
         * @param row the current data row being processed
         ...
         * @return list of data cells, containing the parsed data
         */
        DataCell[] handleFollowingResponse(final DataRow row, final Response response, final MissingCell missing);
