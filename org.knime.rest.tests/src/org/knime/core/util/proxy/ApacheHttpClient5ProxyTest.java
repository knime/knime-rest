...
import org.knime.core.util.proxy.apache5.ProxyConnectionReuseStrategy;
...
import org.knime.core.util.proxy.apache5.ProxyHttpRoutePlanner;
 ...
 * {@link ProxyConnectionReuseStrategy} on standard Apache {@link HttpClient}s 5.X.
 */
class ApacheHttpClient5ProxyTest {
    ...
    private static CloseableHttpClient createClient(final Consumer<HttpClientBuilder> configurer) {
        final var timeout = Timeout.ofSeconds(TIMEOUT_SECONDS);
    ...
    void testRoutePlanner(final ProxyTestContext context) throws IOException {
        ...
        context.withEmpty(() -> {
            try (var client = createClient(b -> b.setRoutePlanner(ProxyHttpRoutePlanner.INSTANCE))) {
                client.execute(target, response -> {
                    throw new AssertionError(
                        "Should not have reached here, request should throw an UnknownHostException");
                });
    ...
    void testCredentialsProvider(final ProxyTestContext context) throws IOException {
        ...
        final var target = HttpbinTestContext.getHttpRequest5("https", "GET");
        ...
        try (var client = createClient(b -> ProxyHttpClients.customForBuilder(b, toURI(target)))) {
            context.withConfig(proxy, () -> {
                client.execute(target, response -> {
                    assertThat(response.getCode()) //
    ...
    void testNoConnectionReuse(final ProxyTestContext context) throws IOException {
        ...
        try (var manager = PoolingHttpClientConnectionManagerBuilder.create()
                ...
                .setDefaultConnectionConfig(ConnectionConfig.custom().setTimeToLive(-1, TimeUnit.MILLISECONDS).build())
                .build();
             var client = createClient(b -> ProxyHttpClients.customForBuilder(b, toURI(target)) //
    ...
    private static URI toURI(final ClassicHttpRequest target) {
        try {
