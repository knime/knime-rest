...
import org.knime.core.util.proxy.EclipseProxyTestContext;
 ...
 *
 * Yes, this is technically the wrong test package those classes, but we want to test Eclipse-based proxies using the
 * {@link EclipseProxyTestContext} which is only possibly while using Tycho.
 */
class ApacheHttpClientProxyTest {
    ...
    @Test
    void testRoutePlanner() throws IOException, CoreException {
        ...
        EclipseProxyTestContext.withEmpty(() -> {
        ...
        EclipseProxyTestContext.withConfig(proxy, () -> {
            try (var client = createClient(b -> b.setRoutePlanner(ProxyHttpRoutePlanner.INSTANCE));
                    var response = client.execute(target)) {
        ...
        EclipseProxyTestContext.withConfig(
    ...
    @Test
    void testCredentialsProvider() throws IOException, CoreException {
    ...
    @Test
    void testNoConnectionReuse() throws IOException, CoreException {
        // this target is always reachable (not only behind proxy)
        final var target = HttpbinTestContext.getHttpRequest("https", "GET");
